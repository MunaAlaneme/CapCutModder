...
function ScriptCompTmp:onEvent(sys,event)
    if event.args:get(0) == "effects_adjust_speed" then
        ...
        self.pass2material:setFloat("speed",intensity*20)
    end
    if event.args:get(0) == "effects_adjust_intensity" then
        local intensity = event.args:get(1)
        self.pass2material:setFloat("scope",intensity*0.1)
    end

    if event.args:get(0) == "effects_adjust_size" then
        local intensity = event.args:get(1)
        self.pass2material:setFloat("rate",intensity*2.5)
    end
    if event.args:get(0) == "effects_adjust_speed2" then
        local intensity = event.args:get(1)

        self.pass2material:setFloat("speedv",intensity*20)
    end
    if event.args:get(0) == "effects_adjust_intensity2" then
        local intensity = event.args:get(1)
        self.pass2material:setFloat("scopev",intensity*0.1)
    end

    if event.args:get(0) == "effects_adjust_size2" then
        local intensity = event.args:get(1)
        self.pass2material:setFloat("ratev",intensity*2.5)
    end
    if event.args:get(0) == "effects_adjust_speed3" then
        local intensity = event.args:get(1)

        self.pass2material:setFloat("broken",intensity)
    end
    if event.args:get(0) == "effects_adjust_intensity3" then
        local intensity = event.args:get(1)
        self.pass2material:setFloat("broken2",intensity*100.0)
    end

    if event.args:get(0) == "effects_adjust_size3" then
        local intensity = event.args:get(1)
        self.pass2material:setFloat("broken3",intensity*100.0)
    end
end
